      ...
      this.$get = ['$rootScope', '$ionicPopup', '$ionicModal', 'IonicDatepickerService', '$q',
        function ($rootScope, $ionicPopup, $ionicModal, IonicDatepickerService, $q) {
          ...
          $scope.nextMonth = function () {
            if (!(($scope.firstDayEpoch + 86400000) > $scope.toDate)) {
          ...
          function preventOutsideLimit(date) {
            if ($scope.fromDate > date) {
              date = $scope.fromDate
            } else if ($scope.toDate < date) {
              date = $scope.toDate
            }
            return date
          }

          function preventWeekend(date) {
            ...
            date = new Date(preventOutsideLimit(date))
            return date.getTime()
          }
